...
import eramo.amtalek.domain.model.products.orders.DiscountFeesModel
import eramo.amtalek.domain.model.products.orders.OrderDetailsModel
...
data class OrderDetails(
    ...
    fun toOrderDetailsModel(): OrderDetailsModel {
        return OrderDetailsModel(
            ...
            compressorSn ?: "",
            supplierName ?: "",
            if (LocalUtil.isEnglish()) manufacturerEnName ?: "" else manufacturerArName ?: "",
            if (LocalUtil.isEnglish()) modelEnName ?: "" else modelArName ?: "",
            if (LocalUtil.isEnglish()) powerEnName ?: "" else powerArName ?: "",
            if (LocalUtil.isEnglish()) productNameEn ?: "" else productNameAr ?: "",
            modelRkm ?: "",
            skuRkm ?: "",
            status ?: "",
            if (LocalUtil.isEnglish()) mainCatEn ?: "" else mainCatAr ?: "",
            if (LocalUtil.isEnglish()) subCatEn ?: "" else subCatAr ?: "",
            shippingPrice ?: "",
            taxesPrice ?: "",
            allImageDtos
        )
...
data class AllOrders(
    ...
    private fun getPromoCode(): UiText {
        return if (promoCodeFk.equals("0")) UiText.StringResource(R.string.s_dirham, "0")
        else {
            val allSum = allSum?.toFloat()!!
            val percent = promoCodePercent?.toFloat()!!
            val value = (allSum * (percent) / 100)
            UiText.StringResource(R.string.s_dirham, value.toString())
        }
    }
